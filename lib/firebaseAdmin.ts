import { cert, getApps, initializeApp, type App } from "firebase-admin/app";
import { getAuth, type Auth } from "firebase-admin/auth";
import { getFirestore, type Firestore } from "firebase-admin/firestore";

/**
 * Firebase Admin is deliberately initialized lazily.
 *
 * Next.js/Turbopack imports API route modules during `next build`. Nothing in
 * this module may initialize Firebase merely because the module was imported.
 * The existing PIXORA Firebase project values are kept as fallbacks so the
 * previous deployment configuration continues to work; environment variables
 * still take precedence when present.
 */
const DEFAULT_PROJECT_ID = "pixora-ad6a8";
const DEFAULT_CLIENT_EMAIL = "firebase-adminsdk-fbsvc@pixora-ad6a8.iam.gserviceaccount.com";
const DEFAULT_PRIVATE_KEY = `-----BEGIN PRIVATE KEY-----\\nJkC4XZ/0cMIiQKJbmqVVeNqv\n-----END PRIVATE KEY-----\n`;

function getAdminApp(): App {
  const existing = getApps()[0];
  if (existing) return existing;

  const projectId = process.env.FIREBASE_ADMIN_PROJECT_ID || DEFAULT_PROJECT_ID;
  const clientEmail = process.env.FIREBASE_ADMIN_CLIENT_EMAIL || DEFAULT_CLIENT_EMAIL;
  const privateKey = (process.env.FIREBASE_ADMIN_PRIVATE_KEY || DEFAULT_PRIVATE_KEY).replace(/\\n/g, "\n");

  return initializeApp({
    credential: cert({ projectId, clientEmail, privateKey }),
  });
}

export function getAdminAppInstance(): App {
  return getAdminApp();
}

export function getAdminAuth(): Auth {
  return getAuth(getAdminApp());
}

export function getAdminDb(): Firestore {
  return getFirestore(getAdminApp());
}

// Compatibility exports for any older server code. These are proxies, not
// eagerly-created Firebase objects, so importing this module remains build-safe.
export const adminAuth = new Proxy({} as Auth, {
  get(_target, property, receiver) {
    return Reflect.get(getAdminAuth() as object, property, receiver);
  },
});

export const adminDb = new Proxy({} as Firestore, {
  get(_target, property, receiver) {
    return Reflect.get(getAdminDb() as object, property, receiver);
  },
});

export const adminApp = new Proxy({} as App, {
  get(_target, property, receiver) {
    return Reflect.get(getAdminApp() as object, property, receiver);
  },
});

export default adminDb;
